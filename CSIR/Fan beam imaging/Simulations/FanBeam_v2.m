...
ampErrorInt = [0.001 0.05];
b1 = ampErrorInt(1);
b2 = ampErrorInt(2);
nFaultyElms = ceil(0.95*N); % Number of elements with amplitude errors
...
title('Amplitude error of transducer array elements', 'fontsize', szAxLabel2)
xlabel('Elements, n', 'fontsize', szAxLabel1)
ylabel('Amplitude, [dB]', 'fontsize', szAxLabel1)
xlim([1 N]);
h_fig=get(gcf,'CurrentAxes');
set(h_fig, 'fontsize', szAxScale);
axis 'square'
...
Rmin = 3; % Minimum one-way range
Rmax = 4; % Maximum one-way range
...
for i = 1:N % For each receiver element...
    for j = 1:ntargets % ... for each target ...
        ...
        % Received signal:
        stu(i,:) = stu(i,:) + amp_error(i)*(exp(1j*pha).*(td <= Tp & td >= 0));
    end;
end;
...
G = real(stu);
figure('name','Data after zero-padding')
colormap(hot(256));
imagesc(c*t/2, u, G);
title('Data after zero-padding', 'fontsize', szAxLabel2)
...
Sfku = fftshift((fft(Stku.')).',2);

Sfku = Sfku.*(ones(m,1)*(conj(Pf)));

Stku = (ifft(fftshift(Sfku,2).')).';
