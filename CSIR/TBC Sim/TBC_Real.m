...
% This simulation uses real monochrome pulses as opposed to analytical signals
...
dtheta = 0.02; % Angular acquisition interval in degrees
theta_start = 0;
theta_end = 360;
...
s0b = pt;
fs0b = fft(pt);%fty(s0b).*exp(-1j*2*pi*f*R/c);

fsb = fft(stu.').';%fty(stu);    % FFT of baseband echoed signal 
fsmb = fsb.*(ones(m,1)*conj(fs0b)); % Matched filtering and windowing
fsmb = fsmb.*(ones(m,1)*(f <= f(n/2-1))); % Delete the negative frequency component
smb = ifft(fsmb.').'.*(ones(m,1)*exp(-1j*2*pi*fc*t)); % Basebanding
