...
% a train of chirp pulses with different carriers corresponding to the
...
fc = 350e3; % 420e3; % Centre frequency in Hz
B = 50e3; % Total available bandwidth
...
nBeams = 5; % Number of simultaneous beams spanning the unambiguous scanning angle *Must be an odd number
beamThetas = linspace(-thetaScan_min/4, thetaScan_min/4, nBeams); % The beam angles
% beamThetas = [0];
...
% Inter-pulse separation must not be less than the maximum time delay for
% beam steering
dtp = 100*max(abs(delta_tau));

Tpt = nBeams*Tp+(nBeams-1)*dtp; % Length of the pulse train
Ts = rSensing/c - Tp; % Sampling start time
Te = rSensing/c + Tp*1.3; % Sampling end time
n = 2*ceil(max(Te - Ts , 1.2*Tpt )/(2*dt)); % Number of time samples
