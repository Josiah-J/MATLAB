...
% within the operating bandwidth of the transducers. The transmit signal is
% a single pulse designed as the sum of multiple chirp signal with
% different carriers corresponding to the different elevation angles. The 
% effect of the number of transducers and other parameters on the
% beam-steering characteristics can also be investigated.
...
fc = 400e3; % 420e3; % Centre frequency in Hz
B = 5e3; % Total available bandwidth
B0 = B/2; % Total baseband bandwidth
Tp = 500e-6; % Chirp pulse width
...
fs = 20*(fc+B0); % Temporal sampling rate
% fs = 5*B;
dt = 1/fs; % Temporal sampling interval

%% ARRAY AND SIMULTANEOUS BEAMS CONFIGURATION

N = 64; %100; % Number of transmit elements in the array
d = 12.5e-3;  %12.5e-3;  %20e-3;%10.6e-3;% 2*lambda_c; %Inter-element spacing
a = d; %3.8e-3;% Element width
...
nBeams = 1; % Number of simultaneous beams spanning the unambiguous scanning angle *Must be an odd number
beamThetas = 0;%linspace(-thetaScan_min/8, thetaScan_min/8, nBeams); % The beam angles
% beamThetas = [0];
...
b1 = ampErrorInt(1);
b2 = ampErrorInt(2);
nFaultyElms = ceil(0.80*N); % Number of elements with amplitude errors
...
%     smb = fftshift((ifft(fsmb.')).',2);

    smb = (ifft(fsmb.')).';
    smb = smb.*(ones(m,1)*exp(-1j*2*pi*subfc(l)*t)); % Basebanding
    smb = fftshift(smb, 2);
...
for l = 1:nBeams
    ...
    smb = smb + squeeze(dataStack(:,:,l));
end;

G=abs(smb);
xg=max(max(G)); ng=min(min(G)); cg=255/(xg-ng);

figure('name', 'Combined pressure pattern')
colormap(hot(256))
imagesc(c*tm, vecTheta, G);
title('Combined pressure intensity variation with angle at the sensing range', 'fontsize', szAxLabel2)
xlabel('Range, ct [m]', 'fontsize', szAxLabel)
ylabel('Angle, \theta [deg]','fontsize', szAxLabel)
h_fig=get(gcf,'CurrentAxes');
set(h_fig, 'fontsize', szAxScale);
...
aB_normalised = aB./(max(aB));
aB_dB = 20*log10(aB_normalised);
